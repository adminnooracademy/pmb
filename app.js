...
function getMonitoringConfig(){
  ...
  return {
    ...
    fullscreenRequired: cfg.fullscreenRequired !== false,
...
function renderProctorPanel(){
  ...
  return `<div class="proctor-box">
    <b>Monitoring Ringan</b>
    <div class="proctor-grid">
      <div class="proctor-item"><span>Kamera</span><strong id="proctorCamera">${proctorStatusText(p.camera)}</strong></div>
      <div class="proctor-item"><span>Mic</span><strong id="proctorMic">${proctorStatusText(p.mic)}</strong></div>
      <div class="proctor-item"><span>Keluar Tab</span><strong id="proctorTab">${p.tabHidden}x</strong></div>
      <div class="proctor-item"><span>Keluar Fullscreen</span><strong id="proctorFull">${p.fullscreenExit}x</strong></div>
    </div>
    <div class="proctor-mini"><video id="proctorVideo" muted autoplay playsinline></video><small>Preview kamera hanya tampil di perangkat peserta. Sistem tidak merekam video/audio penuh.</small></div>
  </div>`;
}
...
function updateProctorStatus(){
  ...
  const p=compactProctoring();
  if($('proctorCamera')) $('proctorCamera').textContent=proctorStatusText(p.camera);
  if($('proctorMic')) $('proctorMic').textContent=proctorStatusText(p.mic);
  if($('proctorTab')) $('proctorTab').textContent=`${p.tabHidden}x`;
  if($('proctorFull')) $('proctorFull').textContent=`${p.fullscreenExit}x`;
...
async function requestProctorMedia(cfg){
  ...
  try{
    proctorStream=await navigator.mediaDevices.getUserMedia({
      video: needVideo ? {width:{ideal:320}, height:{ideal:240}, frameRate:{ideal:5, max:8}} : false,
      audio: needAudio
    });
...
async function ensureMonitoringReady(selectedExam){
  ...
  const mediaText=[];
  if(cfg.cameraRequired) mediaText.push('kamera wajib aktif'); else if(cfg.cameraOptional) mediaText.push('kamera akan diminta sebagai monitoring ringan');
  if(cfg.micRequired) mediaText.push('mic wajib aktif'); else if(cfg.micOptional) mediaText.push('mic akan diminta tanpa rekaman penuh');
...
function renderProctorSummary(p){
  ...
  return `<div class="proctor-summary"><span class="monitor-badge ${cls}">Indikator: ${risk}</span><span>Tab: ${Number(p.tabHidden||0)}x</span><span>Fullscreen: ${Number(p.fullscreenExit||0)}x</span><span>Copy/Paste: ${Number(p.copy||0)+Number(p.paste||0)}x</span><span>Kamera: ${esc(proctorStatusText(p.camera))}</span><span>Mic: ${esc(proctorStatusText(p.mic))}</span></div>`;
}
...
function mergeResults(...lists){
  ...
  return Array.from(map.values()).sort((a,b)=>String(a.submittedAt||'').localeCompare(String(b.submittedAt||'')));
}
...
function applyRemoteResetIfNeeded(state){
  ...
  const results=getLocalResults().filter(r=>normalizeNoUjian(r.noUjian || r.username || r.name)!==no);
  saveResults(results);
  saveRemoteResults(getRemoteResultsCache().filter(r=>normalizeNoUjian(r.noUjian || r.username || r.name)!==no));
  Object.keys(localStorage).forEach(k=>{
    if(k.includes(no) && (k.startsWith('examDraft:') || k.startsWith('pmbActiveExam:') || k.startsWith('pmbExamPackage:'))) localStorage.removeItem(k);
  });
  localStorage.setItem(participantResetSeenKey(no), resetAt);
  if(!state.package){
    delete currentUser.examPackage;
    currentUser.completedExams=[];
    remoteParticipantState.completedExams=[];
...
function removeParticipantFromLocalCaches(noUjian){
  ...
  saveResults(getLocalResults().filter(r=>normalizeNoUjian(r.noUjian || r.username || r.name)!==no));
  saveRemoteResults(getRemoteResultsCache().filter(r=>normalizeNoUjian(r.noUjian || r.username || r.name)!==no));
}
    ...
    d.innerHTML=`
      <section class="welcome-panel">
        ...
        <div class="welcome-media">
          <img src="assets/banner-pendidikan-5-tahun.png" alt="STIPI Maghfirah Pendidikan 5 Tahun">
        </div>
      </section>
...
function goQuestion(i){if(i<0||i>=currentExam.questions.length)return; currentQuestionIndex=i; persistDraft(); renderQuestion(); renderQGrid(); updateProgress();}
...
function persistDraft(){if(!currentUser || !currentExamKey)return; localStorage.setItem(examStorageKey(),JSON.stringify({examKey:currentExamKey,currentQuestionIndex,answers,flagged,remainingSeconds,startedAt:startedAt?.toISOString(),updatedAt:todayISO(),proctoring:compactProctoring()})); saveActiveExam();}
...
async function submitExam(auto){
  ...
  saveLoginSession();
  if(SHEETS_WEB_APP_URL){try{await fetch(SHEETS_WEB_APP_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({type:'result', participantKey:currentUser.participantKey || currentUser.noUjian || currentUser.username, ...result})});}catch(e){console.warn('Gagal kirim ke Sheets',e)}}
...
window.addEventListener('online',()=>{if(currentExam && !examSubmitted){addProctorEvent('online','Koneksi tersambung kembali',false); persistDraft();}});
...
function renderAdmin(){
  ...
  $('dashboard').innerHTML=`
    ...
    <section class="admin-reset-panel">
      <div>
        <h2>Reset Peserta</h2>
        <p>Gunakan hanya jika peserta benar-benar perlu mengerjakan ulang. Reset akan membuka kunci paket dan menghapus hasil peserta tersebut dari Google Sheets.</p>
      </div>
      <div class="reset-form">
        <input id="resetNoUjian" class="input" placeholder="Masukkan nomor ujian, contoh PMB001">
        <button class="btn btn-danger" id="resetParticipantBtn">Reset Peserta</button>
      </div>
    </section>
  ...
  $('resetParticipantBtn').onclick=async()=>{
    ...
    const ok=await showActionModal({
      title:'Reset Peserta',
      html:`<p>Peserta dengan nomor ujian <b>${esc(no)}</b> akan dibuka kuncinya.</p><p class="modal-warning">Hasil peserta ini di Google Sheets akan dihapus agar peserta bisa mengerjakan ulang. Tindakan ini tidak bisa dibatalkan.</p>`,
      confirmText:'Reset Peserta',
      cancelText:'Batal'
    });
    if(!ok) return;
    const btn=$('resetParticipantBtn');
    btn.disabled=true; btn.textContent='Memproses...';
    try{
      await resetParticipantRemote(no);
      await syncResultsFromSheets(false).catch(()=>{});
      await showAlertMessage('Reset Berhasil', `Peserta ${no} sudah di-reset. Minta peserta logout, refresh halaman, lalu login ulang.`);
      renderAdmin();
    }catch(e){
      btn.disabled=false; btn.textContent='Reset Peserta';
      await showAlertMessage('Reset Gagal', e.message || 'Reset peserta gagal.');
    }
  };
  document.querySelectorAll('[data-reset-participant]').forEach(btn=>{
    btn.addEventListener('click', async()=>{
      const no=btn.dataset.resetParticipant;
      $('resetNoUjian').value=no;
      $('resetParticipantBtn').click();
    });
  });
...
function renderResultsTable(results){return `<table class="results-table"><thead><tr><th>Waktu</th><th>Peserta</th><th>Ujian</th><th>Nilai</th><th>Benar</th><th>Durasi</th><th>Monitoring</th><th>Aksi</th></tr></thead><tbody>${results.slice().reverse().map(r=>{const no=normalizeNoUjian(r.noUjian||r.username);return `<tr><td>${r.submittedAt?new Date(r.submittedAt).toLocaleString('id-ID'):'-'}</td><td><b>${esc(r.name)}</b><br><small>${esc(no||r.username)}</small></td><td>${esc(r.examTitle)}</td><td><b>${r.score}</b></td><td>${r.correct}/${r.total}</td><td>${Math.round((r.durationSeconds||0)/60)} menit</td><td>${renderProctorSummary(r.proctoring || {tabHidden:r.lostFocus||0})}</td><td><button class="btn btn-danger mini-btn" data-reset-participant="${esc(no)}">Reset</button></td></tr>`}).join('')}</tbody></table>`}
function exportAllResults(){
  const results=getResults();
  const rows=[['id','noUjian','username','name','package','exam','score','correct','wrong','total','startedAt','submittedAt','durationSeconds','autoSubmitted','tabHidden','fullscreenExit','copy','paste','contextMenu','offline','camera','mic']];
  results.forEach(r=>{
    const p=r.proctoring||{};
    rows.push([r.id,r.noUjian||r.username||'',r.username,r.name,r.examPackage||'',r.examTitle,r.score,r.correct,r.wrong,r.total,r.startedAt,r.submittedAt,r.durationSeconds,r.autoSubmitted,Number(p.tabHidden||r.lostFocus||0),Number(p.fullscreenExit||0),Number(p.copy||0),Number(p.paste||0),Number(p.contextMenu||0),Number(p.offline||0),p.camera||'',p.mic||'']);
  });
